...
namespace TerrainPloughTools
{
    ...
    internal struct ApplyPaintBrushParrallelJob : IJobParallelFor
    {
        /// <summary>
        /// Heightmap region under brush or simply brushmap.
        /// </summary>
        public NativeArray<float> nAlphamap;
        [ReadOnly]
        ...
        public float dt;
        [ReadOnly]
        public int alphamapSliceIndex;
        [ReadOnly]
        public int depth;

        int3 OneToThree(int index, int xSize, int ySize, int zSize) {
        ...
        /// </summary>
        /// <param name="index">Current index of the texel.</param>0
        public void Execute(int index) {
            // converting 1D index to 2D indexes.
            var index3 = OneToThree(index, brushmapPitch, brushmapPitch, depth);
            int x = index3.x;
            int y = index3.y;
            int z = index3.z;
            ...
            float currentWeight = nAlphamap[index];
            if (z == alphamapSliceIndex) {
                nAlphamap[index] = math.min(math.lerp(currentWeight, 1f, hardness * dt * mask), 1f);
            } else {
                nAlphamap[index] = math.min(math.lerp(currentWeight, 0f, hardness * dt * mask), 1f);
            }
        }
