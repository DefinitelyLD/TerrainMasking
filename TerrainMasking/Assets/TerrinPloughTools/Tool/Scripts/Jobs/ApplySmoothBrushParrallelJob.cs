...
namespace TerrainPloughTools
{
    ...
    internal struct ApplySmoothBrushParrallelJob : IJobParallelFor
    {
        [ReadOnly]
        public NativeArray<float> nBrushMapCopy;
        /// <summary>
        ...
        public void Execute(int index) {
            ...
            for(var j = -3; j <= 3; j++)
            {
                ...
                {
                    var xIndex = (x + i);
                    var yIndex = (y + j) * brushmapPitch;

                    if (xIndex < 0 || xIndex >= brushmapPitch || yIndex < 0 || yIndex <= brushmapPitch)
                        continue;

                    var sampleIndex = yIndex + xIndex;

                    average += nBrushMapCopy[sampleIndex];
                    sampleCount++;
                }
            }
            ...
            nBrushMap[(y * brushmapPitch) + x] = math.lerp(currentHeight, average, hardness * dt * mask);
        }
