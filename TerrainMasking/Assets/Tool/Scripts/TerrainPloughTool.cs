...
namespace TerrainPloughTools {
    ...
    public sealed class TerrainPloughTool : MonoBehaviour {
        ...
        private Texture2D _waterCurrentTexture;
        private Texture2D _waterFoamMaskTexture;
        ...
        [DebugInfoAttribute]
        public Vector3 Flow = new Vector3(1, 1, 1);
        [DebugInfoAttribute]
        public int WaterfallSplashFoamDistance = 32;
        [DebugInfoAttribute]
        public int WaterfallSafeFoamDistance = 4;
        [DebugInfoAttribute]
        public int WaterBoundryWidth = 16;
        #region UNITY_EVENTS
        ...
        private void LateUpdate() {
            ...
            if (_waterMesh != null)
            {
                ...
                GameObject.Find("Ice").GetComponent<MeshFilter>().mesh = _waterMesh;

            }

            if (_waterCurrentTexture != null)
            {
                GameObject.Find("Pool").GetComponent<WaterSurface>().largeCurrentMap = _waterCurrentTexture;
                GameObject.Find("Pool").GetComponent<WaterSurface>().largeCurrentRegionExtent = new Vector2(WATER_SIZE.x, WATER_SIZE.y);
                GameObject.Find("Pool").GetComponent<WaterSurface>().largeCurrentRegionOffset = new Vector2(WATER_SIZE.x * 0.5f, WATER_SIZE.y * 0.5f);

                GameObject.Find("Pool").GetComponent<WaterSurface>().simulationFoamMask = _waterFoamMaskTexture;
                GameObject.Find("Pool").GetComponent<WaterSurface>().simulationFoamMaskExtent = new Vector2(WATER_SIZE.x, WATER_SIZE.y);
                GameObject.Find("Pool").GetComponent<WaterSurface>().simulationFoamMaskOffset = new Vector2(WATER_SIZE.x * 0.5f, WATER_SIZE.y * 0.5f);
            }
        }
        ...
        private void OnDestroy() {
            ...
            if (_waterCurrentTexture != null)
            {
                Destroy(_waterCurrentTexture);
                _waterCurrentTexture = null;
            }
        ...
        private Mesh GenerateWaterMesh(bool[,] waterGrid)
        {
            ...
            var currents = new Color[height, width];

            if (_waterCurrentTexture == null)
            {
                _waterCurrentTexture = new Texture2D(width, height, TextureFormat.RGBA32, false, true, false);
            }
            if(_waterFoamMaskTexture == null) {
                _waterFoamMaskTexture = new Texture2D(width, height, TextureFormat.RGBA32, false, true, false);
            }
            ...
            for (int z = 0; z < height; z++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    float rightHeight = topRightHeight;
                    float leftHeight = 0;
                    float topHeight = 0;
                    float bottomHeight = bottomLeftHeight;
                    float myHeight = topLeftHeight;

                    if(z != 0) {
                        topHeight = heights[z - 1, x] * Terrain.terrainData.size.y;
                    }
                    if(x != 0) {
                        leftHeight = heights[z, x - 1] * Terrain.terrainData.size.y;
                    }

                    rightHeight -= myHeight;
                    leftHeight -= myHeight;
                    topHeight -= myHeight;
                    bottomHeight -= myHeight;

                    var neighbourHeights = new float[4];
                    neighbourHeights[0] = topHeight;
                    neighbourHeights[1] = bottomHeight;
                    neighbourHeights[2] = leftHeight;
                    neighbourHeights[3] = rightHeight;

                    if (Mathf.Approximately(rightHeight, 0f) && Mathf.Approximately(topHeight, 0f) &&
                        Mathf.Approximately(bottomHeight, 0f) && Mathf.Approximately(leftHeight, 0f)) {

                        _waterFoamMaskTexture.SetPixel(x, z, new Color(0f, 0, 0));
                    }

                    var maxHeight = neighbourHeights.Min();
                    if(Mathf.Approximately(maxHeight, 0f)) {
                        currents[z, x] = new Color(0, 0, 0);
                    }
                    else if (Mathf.Approximately(maxHeight, topHeight))
                    {
                        currents[z, x] = new Color(0.5f, 0f, 1f);

                        _waterFoamMaskTexture.SetPixel(x, z, new Color(Mathf.Abs(topHeight), 0, 0));

                    } else if (Mathf.Approximately(maxHeight, bottomHeight))
                    {
                        currents[z, x] = new Color(0.5f, 1f, 1f);
                        _waterFoamMaskTexture.SetPixel(x, z, new Color(Mathf.Abs(bottomHeight), 0, 0));

                    } else if (Mathf.Approximately(maxHeight, leftHeight))
                    {
                        currents[z, x] = new Color(0f, 0.5f, 1f);

                        _waterFoamMaskTexture.SetPixel(x, z, new Color(Mathf.Abs(leftHeight), 0, 0));
                    }
                    else
                    {
                        currents[z, x] = new Color(1f, 0.5f, 1f);

                        _waterFoamMaskTexture.SetPixel(x, z, new Color(0.5f, 0, 0));

                        _waterFoamMaskTexture.SetPixel(x, z, new Color(Mathf.Abs(rightHeight), 0, 0));
                    }

                    //currents[z, x] = new Color(Flow.x, Flow.y, Flow.z);

                    if (Input.GetKey(KeyCode.Space))
                    {
                        print($"Top Right: {topRightHeight}, Top Left: {topLeftHeight}, Bottom Right: {bottomRightHeight}, Bottom Left: {bottomLeftHeight}");
                    }
                    
                    _waterCurrentTexture.SetPixel(x, z, currents[z,x]);
                    ...
                    float stepX = WATER_SIZE.x / width;
                    float stepZ = WATER_SIZE.x / width;
                    ...
                    else if (_waterArea[z, x + 1] == false)
                    {
                        ...
                        for (var i = 0; i < WaterBoundryWidth; ++i) {
                            if (_waterArea[z, x + i] == false) {
                                heights[z, x + i] = wallHeight / Terrain.terrainData.size.y;
                            }
                        }
                    }
                    if (x == 0)
                    ...
                    else if (_waterArea[z + 1, x] == false)
                    {
                        ...
                        for (var i = 0; i < WaterBoundryWidth; ++i) {
                            if (_waterArea[z + i, x] == false) {
                                heights[z + i, x] = wallHeight / Terrain.terrainData.size.y;
                            }
                        }
                    }
                    ...
                    float bottomRightPosY = downBottomRight? 0 : baseHeight;
                    float topRightPosY = downTopRight? 0 : baseHeight;
                    float topLeftPosY = downTopLeft? 0 : baseHeight;
                    float bottomLeftPosY = downBottomLeft ? 0 : baseHeight;

/*                    baseHeight += 0.2f;

                    if (Mathf.Approximately(bottomRightPosY, baseHeight)) {
                        heights[z + 1, x + 1] = (baseHeight / Terrain.terrainData.size.y);
                    }
                    if (Mathf.Approximately(topRightPosY, baseHeight)) {
                        heights[z, x + 1] = (baseHeight / Terrain.terrainData.size.y);
                    }
                    if (Mathf.Approximately(topLeftPosY, baseHeight)) {
                        heights[z, x] = (baseHeight / Terrain.terrainData.size.y);
                    }
                    if (Mathf.Approximately(bottomLeftPosY, baseHeight)) {
                        heights[z + 1, x] = (baseHeight / Terrain.terrainData.size.y);
                    }*/
                    ...
                    vertices.Add(new Vector3(posX + stepX, bottomRightPosY, posZ));     // Bottom-right
                    vertices.Add(new Vector3(posX + stepX, topRightPosY, posZ - stepZ)); // Top-right
                    vertices.Add(new Vector3(posX, topLeftPosY, posZ - stepZ));     // Top-left
                    ...
                    uvs.Add(new Vector2((float)x / width, (float)z / height));         // Bottom-right
                    uvs.Add(new Vector2((float)x / width, (float)(z + 1) / height)); // Top-right
                    uvs.Add(new Vector2((float)(x + 1) / width, (float)(z + 1) / height)); // Top-left
                    uvs.Add(new Vector2((float)(x + 1) / width, (float)z / height));         // Bottom-left
                }
            }
/*            for (int i = 0; i < width; ++i) {
            ...
            _waterCurrentTexture.Apply();
            _waterFoamMaskTexture.Apply();
